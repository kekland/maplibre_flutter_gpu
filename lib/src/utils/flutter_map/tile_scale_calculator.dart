...
class TileScaleCalculator {
  ...
  TileScaleCalculator({required this.crs, required this.tileDimension});

  /// Returns true to indicate that the TileDimensionCache should get replaced.
  bool shouldReplace(Crs crs, int tileDimension) => this.crs != crs || this.tileDimension != tileDimension;
  ...
  double scaledTileDimension(double currentZoom, int tileZoom) {
    ...
    return _cache.putIfAbsent(tileZoom, () => _scaledTileDimensionImpl(currentZoom, tileZoom));
  }

  double _scaledTileDimensionImpl(double currentZoom, int tileZoom) {
    return tileDimension * (crs.scale(currentZoom) / crs.scale(tileZoom.toDouble()));
  }
}
